...
class Training:
    ...
    def _compute_caption_loss_and_acc(self, cnn_model, decoder, batch_seq, batch_img, loss_fn, training=True):
        encoder_out = cnn_model(batch_img)
    ...
    @tf.function    
    def train_step(self, batch_data, cnn_model, decoder, optimizer, loss_fn):
        ...
        with tf.GradientTape() as tape:
            loss, _ = self._compute_caption_loss_and_acc(
                    cnn_model, decoder, batch_seq[:, :], batch_img, loss_fn, training=True
                )
        ...
        train_vars = (
                cnn_model.trainable_variables + decoder.trainable_variables
            )
    ...
    def train(self):
        ...
        # encoder = TransformerEncoderBlock(embed_dim=self.params.embedding_dim, dense_dim=self.params.ff_dim, num_heads=2)
        decoder = TransformerDecoderBlock(embed_dim=self.params.embedding_dim, ff_dim=self.params.ff_dim, num_heads=2, vocab_size=self.params.vocab_size, max_len=self.max_length)
        ...
        for epoch in range(self.params.epochs):
            ...
            for (batch, (img_tensor, target)) in tqdm(enumerate(dataset)):
                ...
                batch_loss, _ = self.train_step(
                    (img_tensor, target), cnn_model, decoder, optimizer, loss_object)
                # total_loss += t_loss
...
class Image2Latex_load:
    ...
    def __init__(self, model_name: str, working_path: str = ""):
        self.model_path = os.path.abspath(
            ".") + "\\trained_models_transformers_CNN_2\\" + working_path + model_name
    ...
    def beam_decoder(self, image, beam_width=10, temperature=0.3): 
        ...
        predictions = self.decoder(
                tokenized_caption, img, training=False, mask=mask
            )
        ...
        for i in range(1, self.params.max_len):
            ...
            for r in results:
                ...
                tmp_preds = self.decoder(tmp_tokenized_caption, img, training=False, mask=tmp_mask)
...
# print(*model.predict("C:\\Users\\shace\\Desktop\\1a0ad579d6.png"), sep="\n")