...
def make_fix_size(
    numpy_image, RESIZE_W, RESIZE_H, random_resize: bool, alpha: bool = False
):
    ...
    if img.size[0] <= RESIZE_W and img.size[1] <= RESIZE_H:
        ...
        if random_resize:  #  and random.randint(0, 1) == 0:
            a = 0.55 * min(
                math.floor(RESIZE_W / img.size[0]), math.floor(RESIZE_H / img.size[1])
            )
            #
        ...
        background.paste(
            img,
            (
                int(RESIZE_W / 2) - int(img.size[0] / 2),
                int(RESIZE_H / 2) - int(img.size[1] / 2),
            ),
        )
        ...
        # return None
        print(
            f"image is too big! w = {img.size[0]} ; h = {img.size[1]}"
        )  # This can't happen
...
def compile_handwritten_image(numpy_background, numpy_formula):
    ...
    resize_coeff = random.choice(
        np.arange(
            1 / 3,
            min(backgr.size[0] / formula.size[0], backgr.size[1] / formula.size[1]),
        )
    )

    formula = formula.resize(
        (int(formula.size[0] * resize_coeff), int(formula.size[1] * resize_coeff))
    )

    backgr.paste(
        formula,
        (
            int(backgr.size[0] / 2) - int(formula.size[0] / 2),
            int(backgr.size[1] / 2) - int(formula.size[1] / 2),
        ),
        formula,
    )
