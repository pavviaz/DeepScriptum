...
def save_image(id, caption):
    ...
    try:
        subprocess.run(["pdflatex", "--shell-escape", "--enable-write18", f"{id}.tex"], timeout=5)
    except Exception as e:
        with open("errors.txt", "a") as err:
            err.write(f"ERROR: {e} with cap {caption} and id {id} at {datetime.now()}\n")
...
def remove_bad_captions():
    with open(LABEL_PATH, "r+") as file:
        ...
        with open(os.path.join(DATASET_DIR, "errors.txt")) as err:
            errors = [(t:=re.findall(r"\d+.tex", line.rstrip("\n"))[0])[:t.find(".tex")] for line in filter(lambda x: len(x) > 0, [e for e in err])]
        l = []
