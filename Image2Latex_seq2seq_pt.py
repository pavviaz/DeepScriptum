...
class BahdanauAttention(nn.Module):
    ...
    def forward(self, features, hidden):
        ...
        attention_hidden_layer = (torch.tanh(self.W1(features) +
                                             self.W2(hidden_with_time_axis))).to(self.device)
...
class Decoder(nn.Module):
        ...
        def initial_lstm_state(self, batch_size):
            return torch.zeros((batch_size, self.units), dtype=torch.float32).unsqueeze(0).repeat(2, 1, 1)
...
class Seq2Seq(nn.Module):
    ...
    def forward(self,
        ...
        hidden_state, cell_state = self.decoder.initial_lstm_state(trg.shape[0]).to(self.device)
        ...
        for t in range(1, trg.shape[1]):
            output, hidden_state, cell_state, _ = self.decoder(output, img_features, hidden_state, cell_state)
...
class Training:
    ...
    def train(self):
        ...
        decoder = Decoder(embedding_dim=self.params.embedding_dim, units=self.params.ff_dim, vocab_size=self.params.vocab_size, device=self.device).to(self.device)
        ...
        for epoch in range(self.params.epochs):
            ...
            for (batch, (img_tensor, target)) in tqdm(enumerate(train_dataset)):
                ...
                loss, batch_loss = model(src=img_tensor, trg=target, criterion=criterion, teacher_forcing_ratio=1.0)
...
class Image2Latex_load:
    ...
    def __init__(self, model_name: str, working_path: str = "", device='cpu'):
        self.model_path = os.path.abspath(
            ".") + "\\trained_models_rnn_pt\\" + working_path + model_name
        ...
        if self.loaded:
            ...
            self.cnn_model = Encoder(self.params.embedding_dim, device=self.device).to(device)
            self.decoder = Decoder(embedding_dim=self.params.embedding_dim, units=self.params.ff_dim, vocab_size=self.params.vocab_size, device=self.device).to(device)
    ...
    def greedy_decoder(self, image_path):
        with torch.no_grad():
            ...
            # Generate the caption using the Transformer decoder
            dec_input = torch.unsqueeze(torch.tensor([self.tokenizer.word_index['<start>']]), 0)
            result = []
            for i in range(self.params.max_len):
                output, hidden_state, cell_state, _ = self.decoder(dec_input, features, hidden_state, cell_state)
