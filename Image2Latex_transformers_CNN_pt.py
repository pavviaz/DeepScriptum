...
class LatexDataset(Dataset):
    def __init__(
        self, img_names, caps, resize_H, resize_W, transform=None, random_resize=False
    ):
...
class Training:
    def __init__(self, device, **kwargs):
        ...
        if not self.params.train_from_ckpt:
            ...
            self.logger.info(
                "Params have been loaded successfully for training from checkpoint!"
            )
    ...
    def data_preprocess_from_ckpt(self):
        ...
        if self.params.tokenizer_type == "keras":
            train_seqs = self.params.tokenizer.texts_to_sequences(train_captions)
            ...
            cap_vector = np.array(
                [el.ids for el in self.params.tokenizer.encode_batch(train_captions)]
            )
        else:
            raise ValueError
    ...
    def data_preprocess(self):
        ...
        if self.params.tokenizer_type == "keras":
            ...
            cap_vector = np.array(
                [el.ids for el in self.tokenizer.encode_batch(train_captions)]
            )
        ...
        for epoch in range(self.start_ckpt_id, self.params.epochs):
            ...
            with torch.no_grad():
                for batch, (img_tensor, target) in tqdm(enumerate(val_dataset)):
                    ...
                    val_acc.append(
                        self.accuracy(
                            torch.argmax(val_logits, dim=-1), target[:, 1:]
                        ).item()
                    )

                self.logger.info(
                    f"Epoch {epoch + 1} Val Loss = {np.mean(val_loss)} Accuracy = {np.mean(val_acc).item()}"
                )
...
class Image2Latex_load:
    ...
    def __init__(
        self, model_name: str, working_path: str = "", ckpt_idx=None, device="cpu"
    ):
        self.model_path = (
            os.path.abspath(".")
            + "/trained_models_transformers_pt/"
            + working_path
            + model_name
        )
        ...
        if self.loaded:
            ...
            try:
                if self.params.tokenizer_type == "keras":
                    ...
                    self.tokenizer.enable_padding(
                        pad_token="<pad>", pad_id=0, length=self.params.max_len
                    )
                else:
                    raise ValueError
            ...
            ckpt = Checkpointing(self.params.checkpoint_path, model=self.model)
        ...
        if not self.loaded:
            ...
            try:
                self.dataset_path = self.input(
                    input(f"dataset_path - Default = {self.dataset_path}: "),
                    self.dataset_path,
                )
                self.c_p = self.input(
                    input(f"caption file - Default = {self.c_p}: "), self.c_p
                )
                self.TOKENIZER_TYPE = self.input(
                    input(
                        f"tokenizer type ('keras' of 'hf') - Default = {self.TOKENIZER_TYPE}: "
                    ),
                    self.TOKENIZER_TYPE,
                )
                self.VOCAB_SIZE = int(
                ...
                self.NUM_ENCODER_LAYERS = int(
                    self.input(
                        input(
                            f"NUM_ENCODER_LAYERS - Default = {self.NUM_ENCODER_LAYERS}: "
                        ),
                ...
                self.NUM_DECODER_LAYERS = int(
                    self.input(
                        input(
                            f"NUM_DECODER_LAYERS - Default = {self.NUM_DECODER_LAYERS}: "
                        ),
    ...
    def predict(
        ...
        temperature=0.3,
        postprocess=True,
        beam_return_only_correct=True,
    ):
        if not self.loaded:
            raise ValueError("Model is not loaded!")
        prediction = (
            self.greedy_decoder(image_path=image_path)
            if decoder_type == "greedy"
            else self.beam_decoder(
                image_path,
                beam_width=beam_width,
                temperature=temperature,
                return_only_correct=beam_return_only_correct,
            )
        )
        return (
            prediction[: prediction.index(" <end>")].replace("<start> ", "")
            if postprocess
            else prediction
        )
    ...
    def beam_decoder(
        self, image, beam_width=10, temperature=0.3, return_only_correct=True
    ):
        self.model.eval()
        ...
        with torch.no_grad():
            ...
            if self.params.tokenizer_type == "keras":
                tokenized_caption = keras.preprocessing.sequence.pad_sequences(
                    self.tokenizer.texts_to_sequences([decoded_caption]),
                    padding="post",
                    maxlen=self.params.max_len,
                )[:, :-1]
            elif self.params.tokenizer_type == "hf":
                tokenized_caption = np.expand_dims(
                    np.array(self.tokenizer.encode(decoded_caption).ids), 0
                )[:, :-1]

            _, tgt_mask, _, tgt_padding_mask = self.model.create_mask(
                img_features, tokenized_caption, 0
            )

            tgt_emb = self.model.positional_encoding(
                self.model.tgt_tok_emb(torch.tensor(tokenized_caption))
            )
            src_emb = self.model.positional_encoding(img_features)

            transformers_out = self.model.transformer(
                src_emb,
                tgt_emb,
                None,
                tgt_mask,
            ...
            predictions = (
                (torch.softmax(logits[0, 0, :], -1) / temperature).unsqueeze(0).numpy()
            )
            ...
            results = [
                [
                    obj[0],
                    obj[1],
                    decoded_caption + self.tokenizer.id_to_token(int(obj[1])) + " ",
                ]
                for obj in init
            ...
            for i in range(1, self.params.max_len - 1):
                ...
                for r in results:
                    if self.params.tokenizer_type == "keras":
                        ...
                        tmp_tokenized_caption = np.expand_dims(
                            np.array(self.tokenizer.encode(r[2]).ids), 0
                        )[:, :-1]

                    _, tmp_tgt_mask, _, tmp_tgt_padding_mask = self.model.create_mask(
                        img_features, tmp_tokenized_caption, 0
                    )
                    tmp_tgt_emb = self.model.positional_encoding(
                        self.model.tgt_tok_emb(torch.tensor(tmp_tokenized_caption))
                    )

                    tmp_preds = torch.softmax(
                        self.model.generator(
                            self.model.transformer(
                                ...
                                None,
                                None,
                                torch.tensor(tmp_tgt_padding_mask),
                                None,
                            )
                        ),
                        -1,
                    ...
                    for obj in self.find_n_best(
                        (tmp_preds[0, i, :] / temperature).unsqueeze(0).numpy(),
                        beam_width,
                    ):
                        new_token = (
                            self.tokenizer.index_word[int(obj[1])]
                            if self.params.tokenizer_type == "keras"
                            else self.tokenizer.id_to_token(int(obj[1]))
                        )
                        tmp_res.append([obj[0] + r[0], obj[1], r[2] + new_token + " "])
        ...
        results = [
            res[2] + " <end>" if not "<end>" in res[2] else res[2] for res in results
        ]
    ...
    def greedy_decoder(self, image_path):
        ...
        with torch.no_grad():
            ...
            img = (
                self.load_image(image_path, torchvision.transforms.ToTensor())
                .unsqueeze(0)
                .to(self.device)
            )
            ...
            for i in range(self.params.max_len):
                ...
                tgt_emb = self.model.positional_encoding(
                    self.model.tgt_tok_emb(
                        torch.tensor(tokenized_caption).to(self.device)
                    )
                )
                src_emb = self.model.positional_encoding(img_features)
                ...
                transformers_out = self.model.transformer(
                    ...
                    None,
                    None,
                    torch.tensor(tgt_padding_mask).to(self.device),
                    None,
                )
                logits = self.model.generator(transformers_out)
                sampled_token_index = np.argmax(logits.cpu()[0, i, :])
                sampled_token = (
                    self.tokenizer.index_word[sampled_token_index.item()]
                    if self.params.tokenizer_type == "keras"
                    else self.tokenizer.id_to_token(sampled_token_index.item())
                )
        ...
        for img in tqdm(dataset):
            ...
            try:
                pred = (
                    self.predict(
                        decoder_type="greedy",
                        image_path=os.path.join(val_dataset, f"{img['image_id']}.png"),
                    )
                    .replace("  ", " ")
                    .replace(" ##", "")
                    .strip()
                ...
                b = bleu_score.sentence_bleu(
                    references=[img["caption"].split()],
    ...
    def random_predict(self, decoder_type, number=5):
        ...
        for im in tqdm(images):
            image_path = "images_150_val/" + im["image_id"] + ".png"
            plotting.append(
                [
                    im["caption"],
                    self.predict(decoder_type, image_path, temperature=0.9),
        ...
        cols = number / 5
        _, axes = plt.subplots(
            nrows=5, ncols=int(cols + (0 if cols.is_integer() else 1))
        )
...
if __name__ == "__main__":
    ...
    van.train()
    # van.train_from_ckpt()
    # van.testing(val_dataset="images_150_test_rgb", labels="test_dataset.json")
